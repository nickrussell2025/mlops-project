...
class ChurnDataProcessor:
    ...
    def load_and_clean_data(self):
        ...
        if not os.path.exists(f"{self.data_path}/Churn_Modelling.csv"):
            kaggle.api.dataset_download_files('shrutimechlearn/churn-modelling', path=self.data_path, unzip=True)
    ...
    def engineer_features(self):
        ...
        self.df_clean["zero_balance"] = (self.df_clean["Balance"] == 0).astype("float64")
...
def pipeline():
    ...
    trainer, model_uri, run_id, X_test, y_test = model_training(features, target, processor)
